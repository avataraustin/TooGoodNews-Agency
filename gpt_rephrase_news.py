...
def gpt_article_rephrase(txt_content):
  ...
  response = openai.ChatCompletion.create(
    ...
    "content": "You are a ridiculously upbeat news article writer"},
    {"role": "user",
    "content": "Rephrase this into a ridiculously positive and enthusiastically optimistic article. Put a ridiculous and positive spin on the article. Summarize to reduce overall length. Be certain to exaggerate any potentially positive aspects. It should be politically neutral and not anti-conservative. : " + txt_content}
    ]
  )
